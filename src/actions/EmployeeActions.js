...
export const login = (email, password) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `${process.env.PROXY}employee/login-employee`,
      { email, password },
      config
    );
    dispatch({
...
export const employeeVerificationAction =
  (token, email) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(
        `${process.env.PROXY}employee/invite-employee?token=${token}&email=${email}`,
        config
      );
      dispatch({
...
export const EmployeeRegisterAction =
  (token, email, password, gender, phone_number, address, country) =>
  async (dispatch) => {
    try {
      dispatch({
      ...
      const { data } = await axios.post(
        `${process.env.PROXY}employee/register-employee`,
        { token, email, password, gender, phone_number, address, country },
        config
      );
      dispatch({
